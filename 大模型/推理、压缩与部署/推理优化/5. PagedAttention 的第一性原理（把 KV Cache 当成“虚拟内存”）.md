...
```
PagedAttention 通过分页的方式管理 KV Cache，将其拆分为固定大小的 block，
使得 KV Cache 的逻辑地址连续，而物理显存可以是离散分布的。
通过 block table 完成逻辑 block 到物理 block 的映射，
推理系统不再需要为每个请求分配一整块连续显存，
从而显著缓解了在高并发、动态请求场景下的显存碎片和 OOM 问题。
相比传统的连续 KV Cache 管理方式，
PagedAttention 的代价是引入了一次额外的地址间接访问，
增加了 kernel 的复杂度，
带来一定的性能开销，但换来了显存利用率和系统可扩展性的显著提升。
```
